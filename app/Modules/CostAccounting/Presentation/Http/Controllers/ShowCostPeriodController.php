...
class ShowCostPeriodController extends Controller
{
    ...
    {
        abort_unless($request->user()->hasPermission('cost-periods.view'),403); $costPeriod->load(['utilities','rates','variances']);
        return Inertia::render('costs/periods/Show',['period'=>[...$costPeriod->only(['id','year','month','processed_flour_quantity']),'label'=>$costPeriod->label(),'status'=>$costPeriod->status->value,'status_label'=>$costPeriod->status->label(),'utilities'=>$costPeriod->utilities->map(fn($record)=>[...$record->only(['id','total_amount','business_percentage','household_percentage','business_amount','household_amount','physical_consumption','physical_consumption_unit','reference']),'type'=>$record->utility_type->value,'type_label'=>$record->utility_type->label(),'billed_from'=>$record->billed_from->toDateString(),'billed_to'=>$record->billed_to->toDateString(),'paid_at'=>$record->paid_at->toDateString()]),'rates'=>$costPeriod->rates->map(fn($rate)=>[...$rate->only(['id','suggested_rate','manual_rate','effective_rate','calculation_base_quantity','override_reason']),'type'=>$rate->cost_type->value,'type_label'=>$rate->cost_type->label(),'method'=>$rate->method->value]),'variances'=>$costPeriod->variances->map(fn($variance)=>[...$variance->only(['id','cost_type','actual_amount','allocated_amount','variance_amount'])])],'canClose'=>$request->user()->hasPermission('cost-periods.close'),'canReopen'=>$request->user()->hasPermission('cost-periods.reopen')]);
    }
}
