...
class ShowProductionController extends Controller
{
    public function __invoke(Request $request, ProductionOrder $production, ProductionAvailability $availability): Response
    {
        ...
        return Inertia::render('production/Show', [
            'order' => [
                ...$production->only(['id', 'document_number', 'flour_quantity', 'expected_dough_quantity', 'actual_dough_quantity', 'waste_quantity', 'ingredient_cost', 'labor_cost', 'overhead_cost', 'total_cost', 'reversal_reason']),
                ...
                'consumptions' => $production->consumptions->map(fn($line)=>[...$line->only(['id','calculated_quantity','actual_quantity','difference_reason','unit_cost','total_cost']),'item'=>$line->item->name,'presentation'=>$line->presentation->name,'unit'=>$line->unit->code]),
                ...
                'outputs' => $production->outputs->map(fn($line)=>['id'=>$line->id,'product'=>"{$line->presentation->item->name} — {$line->presentation->name}",'quantity'=>$line->quantity,'dough_quantity'=>$line->dough_quantity,'allocated_cost'=>$line->allocated_cost,'unit_cost'=>$line->unit_cost]),
            ...
            'canManage' => $request->user()->hasPermission('production.manage'), 'canComplete' => $request->user()->hasPermission('production.complete'), 'canReverse' => $request->user()->hasPermission('production.reverse'), 'canRequestAuthorization' => $request->user()->hasPermission('authorizations.request'),
        ]);
    }
}
