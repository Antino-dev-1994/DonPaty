...
class CreateNextRecipeVersion
{
    ...
    public function execute(Recipe $recipe, User $creator): RecipeVersion
    {
        return DB::transaction(function () use ($recipe, $creator): RecipeVersion {
            ...
            $source = $recipe->versions()->with(['ingredients', 'batchComponents', 'compatibleProducts.finishingComponents'])->latest('version_number')->firstOrFail();
            ...
            foreach ($source->batchComponents as $component) $version->batchComponents()->create($component->only(['type', 'label', 'item_id', 'quantity_per_batch', 'unit_id', 'amount_per_batch', 'sort_order']));
            ...
            $this->audit->execute('recipes.version_created', $version, $creator, after: $version->load(['ingredients', 'batchComponents', 'compatibleProducts.finishingComponents'])->toArray());
