...
class RecipeFormOptions
{
    ...
    public function get(): array
    {
        return [
            ...
            'products' => ProductPresentation::query()->with('item:id,name')->where('is_active', true)->whereHas('item', fn ($query) => $query->where('type', ItemType::FinishedProduct))->orderBy('name')->get()->map(fn (ProductPresentation $presentation) => ['id' => $presentation->id, 'name' => "{$presentation->item->name} — {$presentation->name}", 'sku' => $presentation->sku]),
