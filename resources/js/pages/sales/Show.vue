...
const plannedPayments = computed(() => (props.sale.payment_plan ?? []).reduce((total: number, payment: any) => total + Number(payment.amount || 0), 0));
const projectedBalance = computed(() => Math.max(0, Number(props.sale.total || 0) - Number(props.sale.advance_applied || 0) - plannedPayments.value));
...
<template>
    ...
    <main class="flex flex-1 flex-col gap-5 p-4 md:p-6">
        ...
        <section v-if="sale.status === 'draft'" class="grid gap-4 lg:grid-cols-2">
            <div class="rounded-xl border bg-card p-4"><h2 class="font-semibold">Confirmar operación</h2><p class="mt-1 text-sm text-muted-foreground">La confirmación afecta inventario, pagos, ingreso, costo y cartera en una transacción.</p><div class="mt-4 grid gap-2 rounded-lg bg-muted/40 p-3 text-sm"><div class="flex justify-between"><span>Total de la venta</span><strong>{{ money(sale.total) }}</strong></div><div v-if="sale.advance_applied" class="flex justify-between"><span>Anticipo aplicado</span><strong>{{ money(sale.advance_applied) }}</strong></div><div class="flex justify-between"><span>Recibes hoy</span><strong>{{ money(plannedPayments) }}</strong></div><div class="flex justify-between border-t pt-2"><span>{{ projectedBalance > 0 ? 'Queda en cartera' : 'Saldo después del pago' }}</span><strong :class="projectedBalance > 0 ? 'text-amber-700' : 'text-emerald-700'">{{ money(projectedBalance) }}</strong></div><p v-if="projectedBalance > 0" class="text-xs text-muted-foreground">Al confirmar se crea una cuenta por cobrar a {{ sale.customer }} por este saldo.</p><p v-else class="text-xs text-muted-foreground">La venta quedará pagada por completo.</p></div><form v-if="canConfirm" class="mt-4" @submit.prevent="confirm.post(`/sales/${sale.id}/confirm`)"><Button type="submit" :disabled="confirm.processing || (belowMinimum && !confirm.price_authorization_id)">Confirmar venta</Button></form></div>
